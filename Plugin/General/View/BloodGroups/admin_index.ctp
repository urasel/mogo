<?php
$this->viewVars['title_for_layout'] = __d('general', 'Blood Groups');
$this->extend('/Common/admin_index');

$this->Html
	->addCrumb('', '/admin', array('icon' => 'home'))
	->addCrumb(__d('general', 'Blood Groups'), array('action' => 'index'));

$this->set('tableClass', 'table table-striped');

$this->append('table-heading');
	$tableHeaders = $this->Html->tableHeaders(array(
		$this->Paginator->sort('id'),
		$this->Paginator->sort('name'),
		$this->Paginator->sort('bn_name'),
		array(__d('croogo', 'Actions') => array('class' => 'actions')),
	));
	echo $this->Html->tag('thead', $tableHeaders);
$this->end();

$this->append('table-body');
	$rows = array();
	foreach ($bloodGroups as $bloodGroup):
		$row = array();
		$row[] = h($bloodGroup['BloodGroup']['id']);
		$row[] = h($bloodGroup['BloodGroup']['name']);
		$row[] = h($bloodGroup['BloodGroup']['bn_name']);
		$actions = array($this->Croogo->adminRowActions($bloodGroup['BloodGroup']['id']));
		$actions[] = $this->Croogo->adminRowAction('', array(
			'action' => 'view', $bloodGroup['BloodGroup']['id']
	), array(
			'icon' => 'eye-open',
		));
		$actions[] = $this->Croogo->adminRowAction('', array(
			'action' => 'edit',
			$bloodGroup['BloodGroup']['id'],
		), array(
			'icon' => 'pencil',
		));
		$actions[] = $this->Croogo->adminRowAction('', array(
			'action' => 'delete',
			$bloodGroup['BloodGroup']['id'],
		), array(
			'icon' => 'trash',
			'escape' => true,
		),
		__d('croogo', 'Are you sure you want to delete # %s?', $bloodGroup['BloodGroup']['id'])
		);
		$row[] = $this->Html->div('item-actions', implode(' ', $actions));
		$rows[] = $this->Html->tableCells($row);
	endforeach;
	echo $this->Html->tag('tbody', implode('', $rows));
$this->end();
