...
if ($this->action == 'admin_edit') {
	$this->Html->addCrumb($this->request->data['Religion']['name'], '/' . $this->request->url);
	$this->viewVars['title_for_layout'] = __d('general', 'Religions') . ': ' . $this->request->data['Religion']['name'];
} else {
	$this->Html->addCrumb(__d('croogo', 'Add'), '/' . $this->request->url);
}

$this->append('form-start', $this->Form->create('Religion'));

$this->append('tab-heading');
	echo $this->Croogo->adminTab(__d('general', 'Religion'), '#religion');
	echo $this->Croogo->adminTabs();
$this->end();

$this->append('tab-content');

	echo $this->Html->tabStart('religion');
