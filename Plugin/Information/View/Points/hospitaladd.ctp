...
<?php echo $this->Form->create('Point',array('type'=>'file')); ?>
	<fieldset>
		<legend><?php echo __('Add Doctor Information'); ?></legend>
	<?php
		...
		echo $this->Form->input('Hospital.image', array('type'=>'file','placeholder'=>'Profile Image','tabindex'=> "9"));
	...
	function callMap(){
			...
			google.maps.event.addListener(map, 'click', function(mEvent) {
				...
				var databody = $('<h3>Marker position is:</h3>' +
				  mEvent.latLng.lat() + ', ' + mEvent.latLng.lng() +'<br/>'+
		...
		$( "#HospitalName" ).autocomplete({
		  minLength: 0,
		  source: '<?php echo $this->base; ?>/points/getPlaces',
		...
		.autocomplete( "instance" )._renderItem = function( ul, item ) {
		  return $( "<li>" )
			.append( "<div class='searchblock'><div class='iconblock'><i class='" + item.icon + "'></i></div><div class='infoblock'>"+ item.label+"<br/><span>"+ item.thana+","+ item.district+",<b>"+ item.division+"</b></span></div></div>")
			.appendTo( ul );
		};
