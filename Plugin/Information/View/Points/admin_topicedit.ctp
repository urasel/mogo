...
$this->append('tab-content');
				...
				<script type="text/javascript" language="javascript">
					...
					control.makeTransliteratable(['TopicData1Name','TopicData1ShortDescription','TopicData1Keyword','TopicData1Metatag']);
					}
					google.setOnLoadCallback(onLoad);
				</script>
				<?php
	//foreach($this->request->data['TopicData'] as $key => $langData){
		//debug($this->request->data);
		//if($this->request->data['TopicData'][$key]['language_id'] == 2){
			echo $this->Html->tabStart('bangla');
				echo $this->Form->input("TopicData.id", array('type'=>'hidden','tabindex'=> "1"));
				echo $this->Form->input("TopicData.bn_name", array('label'=>'Topic Name','tabindex'=> "1","data-validation" => "required"));
				echo $this->Form->input("TopicData.bn_short_description", array('rows'=> 2,'label'=>'Short Description','tabindex'=> "2","data-validation" => "required"));
				echo $this->Form->input("TopicData.bn_details", array('type'=>'textarea','label'=>'Details  [For Language Change Type ctrl+ g]','tabindex'=> "3"));
				echo $this->Form->input("TopicData.bn_keyword", array('label'=>'Keyword','tabindex'=> "5"));
				echo $this->Form->input("TopicData.bn_metatag", array('label'=>'Meta Description','tabindex'=> "6"));
				
			echo $this->Html->tabEnd();
		//}else if($this->request->data['TopicData'][$key]['language_id'] == 1){
			echo $this->Html->tabStart('english');
				echo $this->Form->input("TopicData.name", array('label'=>'Topic Name','tabindex'=> "1"));
				echo $this->Form->input("TopicData.short_description", array('rows'=> 2,'label'=>'Short Description','tabindex'=> "2"));
				//echo $this->Form->input('en.details', array('type'=>'textarea','class'=>'form-control ckeditor','label'=>'Details','tabindex'=> "3"));
				echo $this->Form->input("TopicData.details", array('type'=>'textarea','label'=>'Details  [For Language Change Type ctrl+ g]','tabindex'=> "3"));
				echo $this->Form->input("TopicData.keyword", array('label'=>'Keyword','tabindex'=> "5"));
				echo $this->Form->input("TopicData.metatag", array('label'=>'Meta Description','tabindex'=> "6"));
			echo $this->Html->tabEnd();
		//}else{
	...
	echo $this->Html->tabStart('point');
		...
		if($userData['role_id'] == 1){
			$selectedUser = $this->request->data['TopicData']['user_id'];
			echo $this->Form->input('user_id',array('type'=>'select','options'=>$users,'selected'=> $selectedUser));
		}
