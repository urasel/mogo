...
$this->append('form-start', $this->Form->create('Point',array('type'=>'file','url' => array('action' =>'placeedit') )));
...
$this->append('tab-heading');
	...
	echo $this->Croogo->adminTab(__d('userfeedback', 'User Feedback'), '#userfeed');
...
$this->append('tab-content');
	...
	echo $this->Html->tabStart('place');
		...
		if($this->request->data['PlaceType']['id'] == 11){
			echo $this->Form->input($dataClass.'.extone',array('label' => 'Swiftcode'));	
			echo $this->Form->input($dataClass.'.exttwo',array('label' => 'Routing Number'));	
			echo $this->Form->input($dataClass.'.extthree',array('label' => 'Bank Code'));	
		}
		...
		if($this->request->data['PlaceType']['id'] == 17){
			echo $this->Form->input($dataClass.'.how_to_go');	
			echo $this->Form->input($dataClass.'.where_to_stay');	
			echo $this->Form->input($dataClass.'.travel_tips');	
			echo $this->Form->input($dataClass.'.where_to_eat');	
		}
		/************************************Hours Section Start*********************************/
		...
		echo $this->Form->input($dataClass.'.establish');	
		echo $this->Form->input($dataClass.'.history');
		...
		if(!empty($this->request->data[$dataClass]['logo'])){
			...
			echo $this->Html->image('places/logo/medium/'.$this->request->data[$dataClass]['logo']);
		}
		echo $this->Form->input($dataClass.'.logo',array('type' =>'file','label' =>'Logo'));
		
		/******************************Point Map Operation****************************/
		$filePath = WWW_ROOT.'img'.DS.'placemaps'.DS.$this->request->data['Point']['map'];
		if(!empty($this->request->data['Point']['map']) && file_exists($filePath)){
			$link = 'placemaps/'.$this->request->data['Point']['map'];
			echo '<div id="mapviewblock">';
			echo $this->Html->image($link);
			$pointMapFieldId = $this->request->data['Point']['id'];
			echo "<button class='btn btn-danger pointMapDelbtn' id='$pointMapFieldId' type='button'>Detele Map Image</button>";
			echo '</div>';
			
		}
		
		//echo $this->Form->input('Point.map',array('type' =>'file','label' =>'Place Map'));
		/******************************Point Map Operation End****************************/
		
		/******************************360 Degree Image Operation****************************/
		$filePath = WWW_ROOT.'img'.DS.'360'.DS.$this->request->data[$dataClass]['panaroma'];
		if(!empty($this->request->data[$dataClass]['panaroma']) && file_exists($filePath)){
			$link = 'placemaps/'.$this->request->data[$dataClass]['panaroma'];
			echo '<div id="panaromaviewblock">';
			echo $this->Html->image($link);
			$panaromaRowId = $this->request->data[$dataClass]['id'];
			echo "<button class='btn btn-danger pointMapDelbtn' id='$panaromaRowId' type='button'>Detele Panaroma</button>";
			echo '</div>';
			
		}
		
		echo $this->Form->input($dataClass.'.panaroma',array('type' =>'file','label' =>'Upload 360 Degree Image'));
		/******************************360 Degree Image Operation End****************************/
...
<script type="text/javascript">
	...
	$(document).ready(function() {
			...
			var geocoder = new google.maps.Geocoder();
				  ...
				  geocoder.geocode({
					...
					if (status == google.maps.GeocoderStatus.OK) {
					  ...
					  //document.getElementById('dd').innerHTML = JSON.stringify(result.geometry.location);
		...
		function nearestplaces(lat,lng){
			...
			$.get("savedplaces/"+lat+"/"+lng, function (data) {
					$(data).find("marker").each(function () {
						  ...
						  place_marker(point, name, address, true, true, true, "http://oximap.com/img/marker-ping.png");
					});
				});	
		}
		...
		function callMap(){
			...
			place_marker(mapCenter, name, '', true, true, true, "http://oximap.com/img/marker-ping.png");
			var counter = 0;
			google.maps.event.addListener(map, 'click', function(mEvent) {
				marker.setPosition(mEvent.latLng);
				//marker.setMap(map);
	...
	//debug($this->data);
		if(isset($this->data['PlaceImage']) && count($this->data['PlaceImage']) > 0){
			foreach ($this->data['PlaceImage'] as $image){
				$imageID = 'del-'.$image['id'];
				$imageConID = 'condel-'.$image['id'];
				echo "<div class='tab-pane'  id='$imageConID' style='width:200px;'>";
				$imglink = "places/photogallery/".$image['file'];
		...
		echo $this->Form->input('image', array('type'=>'file','label'=>'Add Image','multiple'=>'multiple','name' =>'data[Place][images][]'));
		?>
		<script>
		...
		$('.pointMapDelbtn').click(function(){
				var delID = $(this).attr('id');
				var modelname = "Point";
				var foldername = "placemaps";
				var filename = "<?php echo $this->data['Point']['map']; ?>";
				var classid = "<?php echo $this->data['Point']['id']; ?>";
				$.ajax({
					dataType: "html",
					type: "POST",
					evalScripts: true,
					url: '<?php echo $this->base; ?>/information/hospital_images/mapajaxdelete',
					data: ({imageid:delID,modelName:modelname,folder:foldername,filename:filename,classid:classid}),
					success: function (data, textStatus){
						$("#mapviewblock").remove();
	...
	echo $this->Html->tabStart('userfeed');
	$updateData = $this->data['UpdateInformation'];
	foreach($updateData as $upInfo){
		//debug($upInfo);
		echo '<p>';
		echo $upInfo['feedback'].'<br/>';
		$verifiedChecbox = "<input type='checkbox' id='' name='updatestatus'/>";
		echo '<b>Notice Date</b>: '.$upInfo['created']."/t Checked : ";
		echo '<br/><hr></hr>';
		echo '</p>';
