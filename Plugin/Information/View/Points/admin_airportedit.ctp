...
$this->append('form-start', $this->Form->create('Point',array('type'=>'file','url' => array('action' =>'airportedit') )));
...
$this->append('tab-content');
	...
	echo $this->Html->tabStart('place');
		...
		echo $this->Form->input($dataClass.'.airport_type_id');	
		...
		echo $this->Form->input($dataClass.'.ident');	
		echo $this->Form->input($dataClass.'.elevation_ft');	
		echo $this->Form->input($dataClass.'.scheduled_service');	
		echo $this->Form->input($dataClass.'.gps_code');	
		echo $this->Form->input($dataClass.'.iata_code');	
		echo $this->Form->input($dataClass.'.local_code');	
		...
		if(!empty($this->request->data[$dataClass]['logo'])){
			...
			echo $this->Html->image('airports/logo/medium/'.$this->request->data[$dataClass]['logo']);
		}
	...
	//debug($this->data);
		if(isset($this->data[$imageClass]) && count($this->data[$imageClass]) > 0){
			foreach ($this->data[$imageClass] as $image){
				...
				$imglink = "airports/photogallery/".$image['file'];
