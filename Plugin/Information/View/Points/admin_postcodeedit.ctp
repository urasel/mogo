...
$this->append('form-start', $this->Form->create('Point',array('type'=>'file','url' => array('action' =>'postcodeedit') )));
...
$this->append('tab-content');
	...
	echo $this->Html->tabStart('place');
		...
		echo $this->Form->input($dataClass.'.title');
		echo $this->Form->input($dataClass.'.bn_title');
		echo $this->Form->input($dataClass.'.post_code');	
		echo $this->Form->input($dataClass.'.bn_post_code');	
		...
		if(!empty($this->request->data[$dataClass]['image'])){
			echo $this->Form->input($dataClass.'.image',array('type'=>'hidden'));
			echo $this->Html->image('postcodes/logo/medium/'.$this->request->data[$dataClass]['image']);
		}
		echo $this->Form->input($dataClass.'.newimage',array('type' =>'file','label' =>'Logo'));
	...
	//debug($this->data);
		if(isset($this->data[$imageClass]) && count($this->data[$imageClass]) > 0){
			foreach ($this->data[$imageClass] as $image){
				...
				$imglink = "postcodes/photogallery/".$image['file'];
