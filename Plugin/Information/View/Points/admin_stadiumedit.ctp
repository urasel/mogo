...
$this->append('form-start', $this->Form->create('Point',array('type'=>'file','url' => array('action' =>'stadiumedit') )));
...
$this->append('tab-content');
	...
	echo $this->Html->tabStart('place');
		...
		echo $this->Form->input($dataClass.'.tenant_or_use');	
		echo $this->Form->input($dataClass.'.city');	
		echo $this->Form->input($dataClass.'.capacity');	
		echo $this->Form->input($dataClass.'.builton');	
		echo $this->Form->input($dataClass.'.seats');	
	...
	//debug($this->data);
		if(isset($this->data[$imageClass]) && count($this->data[$imageClass]) > 0){
			foreach ($this->data[$imageClass] as $image){
				...
				$imglink = "stadia/photogallery/".$image['file'];
