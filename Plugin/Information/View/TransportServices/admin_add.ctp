...
$this->append('tab-content');
	...
	echo $this->Html->tabStart('transport-service');
		...
		<?php
		echo $this->Form->input('country_id', array('label'=>'Country Name','options' => $countries,'id'=>'CountryId','empty'=>'Country Name','tabindex'=> "2"));
	...
	echo $this->Html->tabStart('transport-route');
		...
		echo $this->Form->unlockField('RouteFromName');
		echo $this->Form->unlockField('RouteToId');
		echo $this->Form->unlockField('RouteToName');
		echo '<div class="routeContainer">';
			...
			*/
				...
				echo '<div class="trroutespeccontainer">';
				echo '</div>';
		...
		<script>
		  $( function() {
			...
			$("#CountryId").change(function(){
					...
					$.ajax({
						url: "<?php echo $this->base; ?>/general/zones/ajaxtrzone",
						dataType: "html",
						data: {
							label: 'Route From',
							fieldname: 'route_from',
							country: countryID
						},
						success: function(data) {
							$(".routeFromContainer").html(data);
						}
					});
					$.ajax({
						url: "<?php echo $this->base; ?>/general/zones/ajaxtrzone",
			...
			if($("#TransportServiceSameCountry"). prop("checked") == false){
				...
				$countryID = $('#CountryId').val();
					$.ajax({
						url: "<?php echo $this->base; ?>/general/zones/ajaxonlyzone",
						dataType: "json",
						data: {
							fieldname: '',
			...
			*/
			...
			$('#TransportRouteName').focus(function(){
							...
							$transportName = $('#TransportServiceName').val();
							//$fromStation = $('#TransportRouteRouteFrom option:selected').text();
							$fromStation = $(this).closest('input').find('TransportRouteDepartureTime').val();
							alert($fromStation);
