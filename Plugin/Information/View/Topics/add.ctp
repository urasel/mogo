...
if ($this->action == 'admin_edit') {
	$this->Html->addCrumb($this->request->data['Topic']['id'], '/' . $this->request->url);
	$this->viewVars['title_for_layout'] = __d('information', 'Topics') . ': ' . $this->request->data['Topic']['id'];
} else {
	$this->Html->addCrumb(__d('croogo', 'Add'), '/' . $this->request->url);
}

$this->append('form-start', $this->Form->create('Topic'));

$this->append('tab-heading');
	echo $this->Croogo->adminTab(__d('information', 'Topic'), '#topic');
	echo $this->Croogo->adminTabs();
$this->end();

$this->append('tab-content');

	echo $this->Html->tabStart('topic');
		...
		echo $this->Form->input('reviewedby', array(
			'label' =>  __d('information', 'Reviewedby'),
		));
