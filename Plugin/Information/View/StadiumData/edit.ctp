...
if ($this->action == 'admin_edit') {
	$this->Html->addCrumb($this->request->data['StadiumData']['name'], '/' . $this->request->url);
	$this->viewVars['title_for_layout'] = __d('information', 'Stadium Data') . ': ' . $this->request->data['StadiumData']['name'];
} else {
	$this->Html->addCrumb(__d('croogo', 'Add'), '/' . $this->request->url);
}

$this->append('form-start', $this->Form->create('StadiumData'));

$this->append('tab-heading');
	echo $this->Croogo->adminTab(__d('information', 'Stadium Data'), '#stadium-data');
	echo $this->Croogo->adminTabs();
$this->end();

$this->append('tab-content');

	echo $this->Html->tabStart('stadium-data');
