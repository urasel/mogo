...
if ($this->action == 'admin_edit') {
	$this->Html->addCrumb($this->request->data['TopicData']['name'], '/' . $this->request->url);
	$this->viewVars['title_for_layout'] = __d('information', 'Topic Data') . ': ' . $this->request->data['TopicData']['name'];
} else {
	$this->Html->addCrumb(__d('croogo', 'Add'), '/' . $this->request->url);
}

$this->append('form-start', $this->Form->create('TopicData'));

$this->append('tab-heading');
	echo $this->Croogo->adminTab(__d('information', 'Topic Data'), '#topic-data');
	echo $this->Croogo->adminTabs();
$this->end();

$this->append('tab-content');

	echo $this->Html->tabStart('topic-data');
