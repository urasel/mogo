		...
		if ($this->request->is('post')) {
			...
			if(isset($this->request->data[$modelName]['image'])){
				...
				if (file_exists($sTempFileName) && filesize($sTempFileName) > 0) {
						...
						$this->Imageresizer->resize($sResultFileName, WWW_ROOT.'img'.DS.'topics'.DS.'small'.DS.$newFilename,73,50,false,false,90);
						$this->Imageresizer->resize($sResultFileName, WWW_ROOT.'img'.DS.'topics'.DS.'medium'.DS.$newFilename,247,170,true,$largewaterimage,80);
						$this->Imageresizer->resize($sResultFileName, WWW_ROOT.'img'.DS.'topics'.DS.'large'.DS.$newFilename,507,350,true,$largewaterimage,80);
		...
		if ($this->request->is(array('post', 'put'))) {
			...
			/*Image Crop Function Start */
				...
				if (file_exists($sTempFileName) && filesize($sTempFileName) > 0) {
						...
						$this->Imageresizer->resize($sResultFileName, WWW_ROOT.'img'.DS.'topics'.DS.'small'.DS.$newFilename,73,50,false,false,100);
						$this->Imageresizer->resize($sResultFileName, WWW_ROOT.'img'.DS.'topics'.DS.'medium'.DS.$newFilename,247,170,true,$largewaterimage,100);
						$this->Imageresizer->resize($sResultFileName, WWW_ROOT.'img'.DS.'topics'.DS.'large'.DS.$newFilename,507,350,true,$largewaterimage,90);
						$this->request->data['Point']['image'] = $newFilename;
						$this->request->data['TopicData']['image'] = $newFilename;
						unset($this->request->data['Crop']);
						@unlink($sResultFileName);
		...
		if ($this->request->is('post')) {
			...
			if(isset($this->request->data[$modelName]['image'])){
				...
				if(!empty($imagefile['tmp_name'])){
					...
					$this->Imageresizer->resize($imagefile['tmp_name'], WWW_ROOT.'img'.DS.'doctors'.DS.$newFilename,250,250,false,false,90);
					$this->request->data[$modelName]['image'] = $newFilename;
					}
				}else{
					unset($this->request->data[$modelName]['image']);
		...
		if ($this->request->is('put')) {
			...
			if(isset($this->request->data[$modelName]['logo'])){
				...
				if(!empty($imagefile['tmp_name'])){
					...
					$this->Imageresizer->resize($imagefile['tmp_name'], WWW_ROOT.'img'.DS.$imageFolder.DS.'logo'.DS.'medium'.DS.$newFilename,250,250,false,false,100);
			...
			if(count($this->request->data[$modelName]['images']) > 0){
				...
				foreach($galleryImages as $imagefile){
					if(!empty($imagefile['tmp_name'])){
						...
						$type = $imagefile['type'];
						if(($type == 'image/gif') || ($type == 'image/jpeg') || ($type == 'image/png') || ($type == 'image/jpg')){
						$this->Imageresizer->resize($imagefile['tmp_name'], WWW_ROOT.'img'.DS.'hospitals'.DS.'photogallery'.DS.$newFilename,400,300,false,false,80);
						$this->Imageresizer->resize($imagefile['tmp_name'], WWW_ROOT.'img'.DS.'hospitals'.DS.'photogallery'.DS.'thumbs'.DS.$newFilename,75,75,false,false,100);
						$this->Imageresizer->resize($imagefile['tmp_name'], WWW_ROOT.'img'.DS.'hospitals'.DS.'photogallery'.DS.'small'.DS.$newFilename,184,140,false,false,100);
						$this->request->data[$imageDB][$imgCount]['point_id'] = $id;
						$this->request->data[$imageDB][$imgCount]['file'] = $newFilename;
						if($imgCount == 0){
						$this->request->data[$modelName]['image'] = $newFilename;
		...
		if ($this->request->is('put')) {
			...
			if(isset($this->request->data[$modelName]['newimage'])){
				...
				if(!empty($imagefile['tmp_name'])){
					...
					$this->Imageresizer->resize($imagefile['tmp_name'], WWW_ROOT.'img'.DS.'postcodes'.DS.'logo'.DS.'medium'.DS.$newFilename,250,250,false,false,100);
					$this->request->data[$modelName]['logo'] = $newFilename;
					}
				}else{
					unset($this->request->data[$modelName]['newimage']);
			...
			if(count($this->request->data[$modelName]['images']) > 0){
				...
				foreach($galleryImages as $imagefile){
					if(!empty($imagefile['tmp_name'])){
						...
						$type = $imagefile['type'];
						if(($type == 'image/gif') || ($type == 'image/jpeg') || ($type == 'image/png') || ($type == 'image/jpg')){
						$this->Imageresizer->resize($imagefile['tmp_name'], WWW_ROOT.'img'.DS.'postcodes'.DS.'photogallery'.DS.$newFilename,749,356,false,false,80);
						$this->Imageresizer->resize($imagefile['tmp_name'], WWW_ROOT.'img'.DS.'postcodes'.DS.'photogallery'.DS.'thumbs'.DS.$newFilename,75,75,false,false,100);
						$this->Imageresizer->resize($imagefile['tmp_name'], WWW_ROOT.'img'.DS.'postcodes'.DS.'photogallery'.DS.'small'.DS.$newFilename,184,140,false,false,100);
						$this->request->data[$imageDB][$imgCount]['point_id'] = $id;
						$this->request->data[$imageDB][$imgCount]['file'] = $newFilename;
						if($imgCount == 0){
						$this->request->data[$modelName]['image'] = $newFilename;
